var VuexORM = (function () {
	...
	***************************************************************************** */
	...
	var Relation = /** @class */ (function (_super) {
	     ...
	     */
	    Relation.prototype.isNormalizedPolymorphicObject = function (record) {
	...
	var HasMany = /** @class */ (function (_super) {
	    ...
	    HasMany.prototype.attach = function (key, record, data) {
	        var _this = this;
	        key.forEach(function (parent) {
	            var relatedRecord = typeof parent === 'object' ? data[parent.schema][parent.id] : data[_this.related.entity][parent];
	            if (!relatedRecord || relatedRecord[_this.foreignKey] !== undefined)
	                return;
	            relatedRecord[_this.foreignKey] = record[_this.localKey];
	        });
	    };
	    /**
	     * Convert given value to the appropriate value for the attribute.
	...
	var HasManyBy = /** @class */ (function (_super) {
	    ...
	    HasManyBy.prototype.attach = function (key, record, _data) {
	        ...
	        record[this.foreignKey] = key.map(function (parent) {
	            var attachment = typeof parent === 'object' ? _data[parent.schema][parent.id] : _data[_this.parent.entity][parent];
	            return _this.parent.getIdFromRecord(attachment);
	        });
	    };
	    /**
	     * Convert given value to the appropriate value for the attribute.
	...
	var MorphMany = /** @class */ (function (_super) {
	    ...
	    MorphMany.prototype.attach = function (key, record, data) {
	        var _this = this;
	        key.forEach(function (related) {
	            var relatedItem;
	            if (typeof related === 'object') {
	                relatedItem = data[_this.related.getTypeModel(related.schema).entity][related.id];
	            }
	            else {
	                relatedItem = data[_this.related.entity][related];
	            }
	            relatedItem[_this.id] = relatedItem[_this.id] || _this.related.getIdFromRecord(record);
	            relatedItem[_this.type] = relatedItem[_this.type] || _this.model.baseEntity || _this.model.entity;
	        });
	    };
	    /**
	     * Convert given value to the appropriate value for the attribute.
	...
	function toAttributes(record) {
	    var model = Model.getModelFromRecord(record);
	    var serializedRecord = toJson(record, { relations: false });
	    serializedRecord.$id = record.$id;
	    if (model.baseEntity && !record[model.typeKey]) {
	        var baseModel = model.database().baseModel(model);
	...
	var Schema = /** @class */ (function () {
	    ...
	    Schema.prototype.many = function (model) {
	        ...
	        if (model.hasTypes() && !model.baseEntity) {
	            var types_1 = model.types();
	            var schema$1 = Object
	                .values(types_1)
	                .map(function (model) {
	                var _a;
	                return (_a = {}, _a[model.entity] = _this.one(model), _a);
	            })
	                .reduce(function (schema, definition) { return Object.assign(schema, definition); }, {});
	            return new schema.Array(__assign(__assign({}, schema$1), (_a = {}, _a[model.entity] = this.one(model), _a)), function (input) { return Object.keys(types_1).includes(input[model.typeKey]) ? types_1[input[model.typeKey]].entity : model.entity; });
	        }
	        else {
	...
	var Normalizer = /** @class */ (function () {
	    ...
	    Normalizer.process = function (query, record) {
	        ...
	        var schema = new Schema(query.model);
	        var normalizerSchema = Utils.isArray(record) ? schema.many(query.model) : schema.one();
	...
	var Loader = /** @class */ (function () {
	    ...
	    Loader.eagerLoadRelations = function (query, collection) {
	        var collections = collection.reduce(function (entities, record) {
	            var entity = query.model.getModelFromRecord(record).entity;
	            if (!entities[entity])
	                entities[entity] = [];
	            entities[entity].push(record);
	            return entities;
	        }, {});
	        for (var entity in collections) {
	            var records = collections[entity];
	            var fields = query.newQuery(entity).model.getFields();
	            var relations = !query.load['*'] ? query.load : Object
	                .keys(fields)
	                .map(function (field) {
	                var _a;
	                return (_a = {}, _a[field] = query.load['*'], _a);
	            })
	                .reduce(function (fields, field) { return Object.assign(fields, field); }, {});
	            for (var name_1 in relations) {
	                var field = fields[name_1];
	                if (field instanceof Relation)
	                    field.load(query, records, name_1, relations[name_1]);
	            }
	        }
	    };
	    return Loader;
	...
	var Query = /** @class */ (function () {
	    ...
	    Query.prototype.getIdsToLookup = function () {
	        ...
	        if (!this.appliedOnBase) {
	            var types_1 = this.baseModel.types();
	            var typeKey_1 = this.baseModel.typeKey;
	            return Object
	                .keys(this.state.data)
	                .filter(function (id) { return types_1[_this.state.data[id][typeKey_1]] && types_1[_this.state.data[id][typeKey_1]].entity == _this.model.entity; });
	        }
	    ...
	    Query.prototype.hydrate = function (record, forceModel) {
	        ...
	        if (!this.appliedOnBase && record[this.baseModel.typeKey] === undefined) {
	            record = __assign(__assign({}, record), (_a = {}, _a[this.baseModel.typeKey] = this.baseModel.getTypeKeyValueFromModel(this.model), _a));
	        }
	        var newModel = this.baseModel.getModelFromRecord(record) || this.getBaseModel(this.entity);
	    ...
	    Query.prototype.hydrateRecordsByMerging = function (records) {
	        ...
	        return Object.keys(records).reduce(function (instances, id) {
	            ...
	            var record = records[id];
	            var modelForRecordInStore = _this.model.getModelFromRecord(record);
